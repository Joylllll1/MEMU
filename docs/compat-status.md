...
| Nanos-lite vfork-style child exec/return | OS/PA4 prework | pass | 2026-07-19 | `make pa-vfork-test`; a finite child is created, execve replaces it, child exit restores the parent's saved stack/context, and the parent prints `parent-after pid=2` |
...
| PA4 final MENU/NWM foreground switching | PA4 | open | 2026-07-19 | Minimal vfork-style child exec and parent resume now pass; the GitBook final demo still needs pipe/dup/dup2/wait semantics and multiple concurrent Navy apps, so this integrated desktop workflow is not claimed complete |
